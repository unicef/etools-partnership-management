...
class AssessmentsItems extends CommonMixin(PolymerElement) {
  static get template() {
    ...
    return html`
      ...
      >
        ...
        <div
          class="row-h no-assessments-warning"
  ...
  _createAssessmentDialog() {
    ...
    this.assessmentDialog.addEventListener(
      "assessment-added",
      this.newAssessmentAdded as any
    );
    this.assessmentDialog.addEventListener(
      "assessment-updated",
      this.assessmentUpdated as any
    );
  ...
  _removeAssessmentDialog() {
    if (this.assessmentDialog) {
      this.assessmentDialog.removeEventListener(
        "assessment-added",
        this.newAssessmentAdded as any
      );
      this.assessmentDialog.removeEventListener(
  ...
  _uploadFinished(e: CustomEvent) {
    ...
    if (e.detail.success) {
      const assessmentIndex = Number(
        (e.target as any).getAttribute("data-args-index")
      ); // TODO - who is e.target
      const uploadResponse = JSON.parse(e.detail.success);
      this.set(
        ["dataItems", assessmentIndex, "report_attachment"],
        uploadResponse.id
      );
