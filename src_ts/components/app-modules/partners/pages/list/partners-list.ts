...
class PartnersList extends connect(store)(
  CommonMixin(
    ListFiltersMixin(
      ListsCommonMixin(
        PaginationMixin(EndpointsMixin(EtoolsCurrency(PolymerElement)))
      )
    )
  )
) {
  static get template() {
    ...
    return html`
      ...
      <style
        include="data-table-styles iron-flex iron-flex-factors paper-material-styles"
      >
        .sm-status-wrapper {
          padding-left: 10px;
        }
        ...
        >
          ...
          >
            <div slot="row-data">
              ...
              <span
                class="col-data flex-3"
                data-col-header-label="Name (Short/Full)"
              >
                <span>[[_computeName(partner.name, partner.short_name)]]</span>

                <span
                  class="sm-status-wrapper"
                  hidden$="[[!partner.deleted_flag]]"
                >
              ...
              <span
                class="col-data flex-2"
                data-col-header-label="Partner Type"
              >
                [[_computeType(partner.cso_type, partner.partner_type)]]
              </span>
              <span
                class="col-data flex"
                data-col-header-label="HACT Risk Rating"
              ...
              <span
                class="col-data flex"
                data-col-header-label="SEA Risk Rating"
                style="text-transform: capitalize"
              >
              ...
              <span
                class="col-data flex"
                data-col-header-label="Last PSEA Assess. Date"
              >
            ...
            <div slot="row-data-details">
              ...
              <div class="row-details-content flex">
                ...
                <span
                  >$ [[displayCurrencyAmount(partner.total_ct_cp, '0')]]</span
                >
              </div>
              <div class="row-details-content flex">
                <span class="rdc-title"
                  >Actual Cash Transfer for Current Year</span
                >
                <span
                  >$ [[displayCurrencyAmount(partner.total_ct_ytd, '0')]]</span
                >
              </div>
            </div>
          </etools-data-table-row>
  ...
  stateChanged(state: RootState) {
    ...
    if (
      !isJsonStrMatch(this.riskRatings, state.commonData!.partnerRiskRatings)
    ) {
      this.riskRatings = [...state.commonData!.partnerRiskRatings];
    }

    if (
      !isJsonStrMatch(this.seaRiskRatings, state.commonData!.seaRiskRatings)
    ) {
  ...
  public _updateSelectedFiltersValues() {
    this._updateShownFilterDebouncer = Debouncer.debounce(
      this._updateShownFilterDebouncer,
      timeOut.after(20),
      () => {
        const filtersValues = [
          {
  ...
  public _init(active: any) {
    ...
    this.setProperties({
      ...
      selectedPartnerTypes: this._getSelectedPartnerTypes(
        urlQueryParams.partner_types
      ),
      selectedCsoTypes: this._getFilterUrlValuesAsArray(
        urlQueryParams.cso_types
      ),
      selectedRiskRatings: this._getFilterUrlValuesAsArray(
        urlQueryParams.risk_ratings
      ),
      selectedSEARiskRatings: this._getFilterUrlValuesAsArray(
        urlQueryParams.sea_risk_ratings
      ),
      selectedPseaDateBefore: urlQueryParams.psea_assessment_date_before
        ? urlQueryParams.psea_assessment_date_before
        : "",
      selectedPseaDateAfter: urlQueryParams.psea_assessment_date_after
  ...
  public _handleFilterPartnersData(forceNoLoading: boolean) {
    const partners = this.shadowRoot!.querySelector(
      "#partners"
    ) as PartnersListDataEl;
    if (!partners) {
      return;
    }
    partners.query(
