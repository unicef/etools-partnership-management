...
} from "@unicef-polymer/etools-ajax/ajax-error-parser.js";
 ...
 */
export class PartnerItemData extends AjaxServerErrorsMixin(
  ...
  public _deletePartnerFromDexie(id: any) {
    window.EtoolsPmpApp.DexieDb.partners
      ...
      .then((deleteCount: number) =>
        this._handlePartnerDeleteFromDexieSuccess(deleteCount)
      )
      .catch((dexieDeleteErr: any) =>
        this._handlePartnerDeleteFromDexieErr(dexieDeleteErr)
      )
      .then(() => {
        // disable loading
        fireEvent(this, "global-loading", {
  ...
  public _handlePartnerDeleteFromDexieErr(dexieDeleteErr: any) {
    ...
    logError(
      "Partner delete from local dexie db failed!",
      "partner-item-data",
      dexieDeleteErr
    );
    fireEvent(this, "toast", {
  ...
  public _handleErrorResponse(response: any, ajaxMethod: any) {
    ...
    if (typeof this.handleErrResponseAdditionalCallback === "function") {
      this.handleErrResponseAdditionalCallback.bind(
        this,
