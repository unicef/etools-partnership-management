 ...
 * @mixinFunction
 */
function LowerResultsMixin<T extends Constructor<PolymerElement>>(
  baseClass: T
) {
  class LowerResultsClass extends baseClass {
    ...
    _initLowerResultsListeners() {
      ...
      this.addEventListener(
        "edit-lower-result",
        this._handleEditLowerResult as EventListenerOrEventListenerObject
      );
    }
    ...
    createLowerResultNameDialog() {
      this.lowerResultNameEditElem = document
        .querySelector("body")!
        .querySelector("#pdLowerResultName") as any;
      if (!this.lowerResultNameEditElem) {
        this.lowerResultNameEditElem = document.createElement(
          "pd-lower-result-name"
        ) as any;
        ...
        this.lowerResultNameEditElem.addEventListener(
          "lower-result-saved",
          this._lowerResultSaved as any
        );
        document
          .querySelector("body")!
          .appendChild(this.lowerResultNameEditElem as any);
      }
    }

    removeLowerResultNameDialog() {
      if (this.lowerResultNameEditElem) {
        this.lowerResultNameEditElem.removeEventListener(
          "lower-result-saved",
          this._lowerResultSaved as any
        );
        document
          .querySelector("body")!
          .removeChild(this.lowerResultNameEditElem as any);
      }
    }
    ...
    _lowerResultSaved(e: CustomEvent) {
      ...
      try {
        if (!e.detail.expectedResultId) {
          logError(
            "Can not make changes if expected result ID is not specified.",
            "lower-results-behavior"
          );
          return;
        }
        // @ts-ignore
        const expectedResultIndex = this._getDataIndex(
          e.detail.expectedResultId,
          this.dataItems
        );
        const validExpectedResultsIndex = expectedResultIndex >= 0;
        if (!validExpectedResultsIndex) {
          logError(
            "Result with ID: " + e.detail.expectedResultId + " not found.",
            "lower-results-behavior"
          );
          return;
        }
        if (!e.detail.lowerResultId) {
          this.push(
            ["dataItems", expectedResultIndex, "ll_results"],
            e.detail.lowerResult
          );
        } else {
          ...
          if (!validIndex) {
            logError(
              "Lower result with ID: " + e.detail.lowerResultId + " not found.",
              "lower-results-behavior"
            );
            return;
          }
          this.set(
            ["dataItems", expectedResultIndex, "ll_results", lrIndex, "name"],
            e.detail.lowerResult.name
          );
        }
      } catch (err) {
        logError(
          "Adding lower result in displayed list has failed!",
          "lower-results-behavior",
          err
        );
      }
    ...
    _handleAddNewLowerResult(e: CustomEvent) {
      e.stopImmediatePropagation();
      this._openLowerResultNameDialog(
        null,
        undefined,
        e.detail.expectedResultId
      );
    ...
    _handleEditLowerResult(e: CustomEvent) {
      ...
      this._openLowerResultNameDialog(
        e.detail.lowerResultId,
        e.detail.lowerResultName,
        e.detail.expectedResultId
      );
    }

    _openLowerResultNameDialog(
      lowerResultId: number | null,
