...
class AgreementSelector extends connect(store)(CommonMixin(PolymerElement)) {
  ...
  stateChanged(state: RootState) {
    ...
    if (
      !isJsonStrMatch(this.partnersDropdownData, csoPartnersSelector(state))
    ) {
  ...
  _setSelectedPartnerId() {
    if (
      ...
      !this.partnersDropdownData.length
    ) {
      return;
    }
    ...
    if (
      this.intervention &&
      this.permissions &&
      !this.permissions.edit.agreement
    ) {
    ...
    if (
      !agreement ||
      !agreement.partner ||
      this.partnerId === agreement.partner
    ) {
      return;
    }
  ...
  _filterAgreements(partnerId: number) {
    ...
    if (
      this.selectedAgreement &&
      this.selectedAgreement.partner !== partnerId
    ) {
  ...
  resetValidations() {
    const agEl = this.shadowRoot!.querySelector(
      "#agreements"
    ) as EtoolsDropdownEl;
    if (agEl) {
  ...
  validate() {
    ...
    if (!this.partnerId) {
      valid = false;
      const pEl = this.shadowRoot!.querySelector(
        "#partner"
      ) as EtoolsDropdownEl;
      if (pEl) {
        pEl.invalid = true;
