...
class PlannedBudget extends connect(store)(
      ...
      <etools-content-panel panel-title="Planned Budget">
        ...
        <div class="list-container form-fields">
          <etools-data-table-header id="listHeader" no-collapse no-title>
            <etools-data-table-column class="col-3 currency">
              Currency</etools-data-table-column
            >
            <etools-data-table-column class="col-3 right-align">
              CSO Contribution</etools-data-table-column
            >
            <etools-data-table-column class="col-2 right-align">
              UNICEF Cash</etools-data-table-column
            >
            <etools-data-table-column class="col-2 right-align">
              UNICEF Supply</etools-data-table-column
            >
            <etools-data-table-column class="col-2 right-align">
              TOTAL</etools-data-table-column
            >
            ...
            <div slot="row-data">
              <span class="col-data col-3 currency">
                ...
                >
                  ...
                  ></iron-icon>
                  <span slot="message"
                    >[[frsConsistencyWarnings.currencyMismatch]]</span
                  >
                </etools-info-tooltip>
              </span>
              ...
              <span class="col-data col-3 right-align">
                <template
                  is="dom-if"
                  if="[[!_isReadonly(editMode, editablePlannedBudget, interventionId)]]"
                >
                  <etools-currency-amount-input
                    id="csoCont"
  ...
  _initLocalCurrency(plannedBudget: any, countryData: Country) {
    ...
    if (
      ...
      countryData &&
      countryData.local_currency_code
    ) {
  ...
  _showEditBtn(
    ...
    return (
      (this.editMode || this.unicefCashEditMode) &&
  ...
  _isCurrencyReadonly(
    ...
    return inAmendmentMode
      ? true
      : this._isReadonly(editMode, editablePlannedBudget, interventionId);
  }
  ...
  _getPlannedBudgetTotalFormatted(val1: any, val2: any, val3: any) {
    return this.displayCurrencyAmount(
      this._getPlannedBudgetTotal(val1, val2, val3),
      "0",
      0
    );
  }
  ...
  _hideBudgetCurrencyMismatchTooltip(
    frsCurrencyMatch: boolean,
    frs: Fr[],
    plannedBudgetCurrency: string
  ) {
    if (!isEmptyObject(frs)) {
      return this.allCurrenciesMatch(
        frsCurrencyMatch,
        frs,
        plannedBudgetCurrency
      );
