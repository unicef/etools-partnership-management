...
class EditHruDialog extends connect(store)(EndpointsMixin(PolymerElement)) {
  static get template() {
    return html`
      ...
      >
        ...
        <div>
          Use the date picker to select end dates of humanitarian report
          requirements.
        </div>
  ...
  _getMinDate() {
    ...
    const stDt =
      this.interventionStart instanceof Date
        ? this.interventionStart
        : convertDate(this.interventionStart);
    if (stDt) {
  ...
  _addToList() {
    const alreadySelected = this.hruData.find(
      (d: any) => d.end_date === this.selectedDate
    );
    if (alreadySelected) {
