 ...
 */
class HumanitarianReportingReqUnicef extends FrontendPaginationMixin(
    ...
    return html`
      ...
      <div hidden$="[[!_empty(reportingRequirements)]]">
        ...
        <div class="row-h" hidden$="[[!_showAdd(expectedResults, editMode)]]">
          <paper-button
            class="secondary-btn"
            on-click="openUnicefHumanitarianRepReqDialog"
          >
            ADD REQUIREMENTS
          </paper-button>
        </div>
        <div class="row-h" hidden$="[[_thereAreHFIndicators(expectedResults)]]">
          Can be modified only if there are High Frequency Humanitarian
          Indicators defined.
        </div>
      </div>
  ...
  _createEditHruDialog() {
    this._reportingRequirementsSaved = this._reportingRequirementsSaved.bind(
      this
    );
    this.editHruDialog = document.createElement("edit-hru-dialog") as any;
    this.editHruDialog.set("toastMsgLoadingSource", this);
    this.editHruDialog.addEventListener(
      "reporting-requirements-saved",
      this._reportingRequirementsSaved as any
    );
  ...
  _removeEditHruDialog() {
    if (this.editHruDialog) {
      this.editHruDialog.removeEventListener(
        "reporting-requirements-saved",
        this._reportingRequirementsSaved as any
      );
  ...
  setTotalResults(interventionId: string, reportingRequirements: any) {
    if (
      typeof interventionId === "undefined" ||
  ...
  _getIndex(index: number) {
    return (
      index + 1 + this.pagination.pageSize * (this.pagination.pageNumber - 1)
    );
  }
...
window.customElements.define(
  ...
  HumanitarianReportingReqUnicef
);
