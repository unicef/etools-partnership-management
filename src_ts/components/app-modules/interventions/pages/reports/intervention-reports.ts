...
class InterventionReports extends PolymerElement {
  ...
  _init(active: boolean) {
    ...
    const page =
      this.prevParams && this.prevParams.page
        ? Number(this.prevParams.page)
        : 1;
    const pageSize =
      this.prevParams && this.prevParams.page_size
        ? Number(this.prevParams.page_size)
        : 10;
  ...
  _updateURL(
    ...
    if (
      !isEmptyObject(this.prevParams) &&
      page === this.prevParams.page &&
    ...
    this._debounceUrlUpdate = Debouncer.debounce(
      this._debounceUrlUpdate,
      timeOut.after(20),
      () => {
        ...
        updateAppState(
          "interventions/" + interventionId + "/reports",
          qs,
          true
        );
        this.set(
          "prevParams",
          Object.assign({}, { page: page, page_size: pageSize })
        );
      }
    );
  }
