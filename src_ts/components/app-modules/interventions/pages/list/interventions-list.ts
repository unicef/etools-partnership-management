...
class InterventionsList extends connect(store)(
  ListFiltersMixin(
    ListsCommonMixin(
      CommonMixin(PaginationMixin(FrNumbersConsistencyMixin(PolymerElement)))
    )
  )
) {
  static get template() {
    return html`
      ${SharedStyles} ${gridLayoutStyles} ${listFilterStyles}
      ${frWarningsStyles}
      ...
      <div id="filters" class="paper-material" elevation="1">
        <div id="filters-fields">
          ...
          <template is="dom-repeat" items="[[selectedFilters]]" as="filter">
            <template
              is="dom-if"
              if="[[filterTypeIs('etools-dropdown-multi', filter.type)]]"
            >
            ...
            <template
              is="dom-if"
              if="[[filterTypeIs('paper-toggle', filter.type)]]"
            >
          ...
          >
            <div slot="row-data" class="p-relative">
              ...
              <span
                ...
                data-col-header-label="Document Type"
              >
                [[getDisplayValue(intervention.document_type)]]
              </span>
              <span
                class="col-data flex-c capitalize"
                data-col-header-label="Status"
              >
              ...
              <span class="col-data flex-c" data-col-header-label="Start Date">
                ...
                >
                  <span slot="field"
                    >[[getDateDisplayValue(intervention.start)]]</span
                  >
                  <iron-icon
              ...
              <span class="col-data flex-c" data-col-header-label="End Date">
                ...
                >
                  <span slot="field"
                    >[[getDateDisplayValue(intervention.end)]]</span
                  >
                  <iron-icon
                    icon="pmp-custom-icons:not-equal"
                    slot="custom-icon"
            ...
            <div slot="row-data-details" class="p-relative">
                ...
                >
                  <span slot="field">
                    <span class="amount-currency"
                      >[[intervention.budget_currency]]</span
                    >
                    <span
                      >[[displayCurrencyAmount(intervention.unicef_cash,
                      '0.00')]]</span
                    >
                  </span>
                  <iron-icon
              ...
              <div class="row-details-content col-2">
                ...
                <span>
                  ...
                  >
                  <span
                    >[[displayCurrencyAmount(intervention.total_budget,
                    '0.00')]]</span
                  >
                </span>
              </div>
  ...
  stateChanged(state: RootState) {
    ...
    if (
      !isJsonStrMatch(
        this.interventionStatuses,
        state.commonData!.interventionStatuses
      )
    ) {
  ...
  _init(active: boolean) {
    ...
    this.setProperties({
      ...
      selectedDocumentTypes: this._getFilterUrlValuesAsArray(
        urlQueryParams.type
      ),
      selectedStatuses: this._getFilterUrlValuesAsArray(urlQueryParams.status),
      selectedCpOutputs: this._getFilterUrlValuesAsArray(
        urlQueryParams.cp_outputs
      ),
      ...
      selectedPartners: this._getFilterUrlValuesAsArray(
        urlQueryParams.partners
      ),
      selectedGrants: this._getFilterUrlValuesAsArray(urlQueryParams.grants),
      selectedUnicefFocalPoints: this._getFilterUrlValuesAsArray(
        urlQueryParams.unicef_focal_points
      ),
      selectedOffices: this._getFilterUrlValuesAsArray(urlQueryParams.offices),
      selectedCPStructures: this._getFilterUrlValuesAsArray(
        urlQueryParams.cpStructures
      ),
    ...
    const result = this.initSortFieldsValues(
      { field: "partner_name", direction: "asc" },
      urlQueryParams.sort
    );
  ...
  _updateSelectedFiltersValues() {
    this._updateFiltersValsDebouncer = Debouncer.debounce(
      this._updateFiltersValsDebouncer,
      timeOut.after(100),
      () => {
        const filtersValues = [
          {
  ...
  _filterListData(forceNoLoading?: boolean) {
    ...
    this._queryDebouncer = Debouncer.debounce(
      this._queryDebouncer,
      timeOut.after(this.debounceTime),
      () => {
        const interventions = this.shadowRoot!.querySelector(
          "#interventions"
        ) as InterventionsListData;
        if (!interventions) {
          return;
        }
        interventions.query(
  ...
  _arrayFilterChanged(filterVal: any, oldFilterVals: any) {
    if (
      typeof oldFilterVals !== "undefined" &&
      filterVal.length !== oldFilterVals.length
    ) {
  ...
  _hideDateFrsWarningTooltip(pdDate: string, frsDate: string, status: string) {
    return !(
      this._canShowListDatesFrsWarnings(status) &&
      !this.validateFrsVsInterventionDates(pdDate, frsDate)
    );
  }
