...
class InterventionsModule extends connect(store)(
      ...
      ModuleMainElCommonFunctionalityMixin(
        ModuleRoutingMixin(
          InterventionPageTabsMixin(
            SaveInterventionMixin(EndpointsMixin(PolymerElement))
          )
        )
      )
    )
    ...
    return html`
      ...
      <page-content-header with-tabs-visible="[[tabsActive]]">
        <div slot="page-title">
          ...
          <template is="dom-if" if="[[tabsActive]]">
            <span
              class="no-capitalization"
              hidden$="[[!newInterventionActive]]"
            >
        ...
        <div
          ...
          class="content-header-actions export-options"
        >
            ...
            <paper-menu-button id="pdExportMenuBtn" close-on-activate>
              ...
              <paper-listbox slot="dropdown-content">
                <paper-item on-tap="_exportPdBudget"
                  >PD Budget Export</paper-item
                >
                <paper-item on-tap="_exportPdResult"
                  >PD Result Export</paper-item
                >
                <paper-item on-tap="_exportPdLocations"
                  >PD Locations Export</paper-item
                >
              </paper-listbox>
            </paper-menu-button>
          </div>
          ...
          <div
            class="action"
            hidden$="[[!_showAddNewIntervBtn(listActive, permissions)]]"
          >
            <paper-button
              class="primary-btn with-prefix"
              on-tap="_goToNewInterventionPage"
            >
      ...
      <div id="main">
        <div id="pageContent">
          ...
          <template
            is="dom-if"
            if="[[_visibleTabContent(activePage, 'overview', newInterventionActive)]]"
          >
          ...
          <template
            is="dom-if"
            if="[[_pageEquals(activePage, 'review-and-sign')]]"
          >
          ...
          <template
            is="dom-if"
            if="[[_visibleTabContent(activePage, 'reports', newInterventionActive)]]"
            restamp
          >
          ...
          <template
            is="dom-if"
            if="[[_visibleTabContent(activePage, 'progress', newInterventionActive)]]"
            restamp
          >
        ...
        <template
          is="dom-if"
          if="[[_showInterventionSidebarStatus(listActive, tabAttached, activePage)]]"
        >
          <div id="sidebar">
            <intervention-status
  ...
  _initInterventionsModuleListeners() {
    ...
    this._handleInterventionSelectionLoadingMsg = this._handleInterventionSelectionLoadingMsg.bind(
      this
    );
    this._refreshInterventionPermissions = this._refreshInterventionPermissions.bind(
      this
    );
    this._amendmentAddedHandler = this._amendmentAddedHandler.bind(this);

    this.addEventListener(
      "intervention-save-error",
      this._interventionSaveErrors as any
    );
    this.addEventListener(
      "tab-content-attached",
      this._interventionPageAttached
    );
    this.addEventListener(
      "trigger-intervention-loading-msg",
      this._handleInterventionSelectionLoadingMsg
    );
    this.addEventListener(
      "refresh-intervention-permissions",
      this._refreshInterventionPermissions as any
    );
    this.addEventListener(
      "new-amendment-added",
      this._amendmentAddedHandler as any
    );
  }
  ...
  _createFinalizeAmendConfirmDialog() {
    this._finalizeAmendmentConfirmationCallback = this._finalizeAmendmentConfirmationCallback.bind(
      this
    );
    const dialog = document.createElement("div");
    dialog.setAttribute("id", "finalizeAmendmentConfirmation");
    dialog.innerHTML =
      "All fields in the details tab will now be closed for editing. Do you want to continue?";
    const conf: any = {
  ...
  _amendmentModeChanged(
    amendmentModeActive: boolean,
    tabAttached: boolean,
    listActive: boolean
  ) {
    if (
      typeof amendmentModeActive === "undefined" ||
      !this.intervention ||
      (!tabAttached && !listActive)
    ) {
      return;
    }
    if (listActive) {
  ...
  _interventionChanged(
    intervention: Intervention,
    permissions: UserPermissions
  ) {
    if (
      typeof intervention === "undefined" ||
    ...
    if (!isEmptyObject(intervention)) {
      ...
      const isNewIntervention =
        this._redirectToNewIntervPageInProgress || this.newInterventionActive;
  ...
  _displayAnyMigrationErrors(intervention: any) {
    if (
      ...
      intervention.metadata.error_msg &&
      intervention.metadata.error_msg.length
    ) {
      if (
        this.saved.interventionId !== intervention.id &&
        !this.saved.justSaved
      ) {
  ...
  _pageChanged(listActive: boolean, tabsActive: boolean, routeData: any) {
    ...
    this._pageChangeDebouncer = Debouncer.debounce(
      this._pageChangeDebouncer,
      timeOut.after(10),
      () => {
  ...
  _updateInterventionStatus(e: CustomEvent) {
    e.stopImmediatePropagation();
    (this.$.interventionData as InterventionItemData).updateInterventionStatus(
      e.detail
    );
  }

  _deleteIntervention(e: CustomEvent) {
    (this.$.interventionData as InterventionItemData).deleteIntervention(
      e.detail.id
    );
  }
  ...
  _userHasEditPermissions(permissions: UserPermissions) {
    return (
      permissions &&
      permissions.editInterventionDetails === true &&
  ...
  _interventionPageAttached() {
    ...
    this._updateRelatedPermStyles(
      false,
      this._forceDetUiValidationOnAttach,
      this._forceReviewUiValidationOnAttach
    );
  }

  _showInterventionSidebarStatus(
    listActive: boolean,
    tabAttached: boolean,
    activePage: string
  ) {
