    ...
    window.EtoolsPmpApp.DexieDb.transaction(
      ...
      function () {
        ...
        queryResult = queryResult.filter(function (
          ...
          if (
            (contingency_pd && !intervention.contingency_pd) ||
            (!contingency_pd && intervention.contingency_pd)
          ) {
            return false;
          }
          ...
          if (
            ...
            startDate.length &&
            (!intervention.start ||
              !moment.utc(intervention.start).isAfter(moment.utc(startDate)))
          ) {
            return false;
          }
          ...
          if (
            ...
            endDate.length &&
            (!intervention.end ||
              !moment.utc(intervention.end).isBefore(moment.utc(endDate)))
          ) {
            return false;
          }
          ...
          if (
            ...
            endAfter.length &&
            (!intervention.end ||
              !moment.utc(intervention.end).isSameOrAfter(moment.utc(endAfter)))
          ) {
            return false;
          }
          ...
          if (
            ...
            intervention.partner_name!.toLowerCase().indexOf(searchString) <
              0 &&
      ...
      .catch(function (error: any) {
        logError(
          "Error querying interventions: " + error,
