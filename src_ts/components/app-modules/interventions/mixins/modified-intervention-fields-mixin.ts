 ...
 * @mixinFunction
 */
function ModifiedInterventionFieldsMixin<T extends Constructor<PolymerElement>>(
  baseClass: T
) {
  class ModifiedInterventionFieldsClass extends baseClass {
    ...
    _primitiveFieldIsModified(fieldName: string) {
      // @ts-ignore
      return (
        this.originalIntervention[fieldName] !== this.intervention[fieldName]
      );
    }
    ...
    _getModifiedInterventionDetails(docType?: string) {
      ...
      const modifiedDetails = this._buildModifiedInterventionObject(
        updatableFields,
        updatableObjectFields
      );

      return Object.assign({}, modifiedDetails);
    }

    _buildModifiedInterventionObject(
      updatableFields: string[],
      updatableObjectFields: string[]
    ) {
