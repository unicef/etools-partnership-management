 ...
 */
function InterventionPageTabsMixin<T extends Constructor<PolymerElement>>(
  baseClass: T
) {
  class InterventionPageTabsClass extends EnvironmentFlagsMixin(
    ...
    _computeInterventionTabs(
      newInterventionActive: boolean,
      environmentFlags: any
    ) {
      if (
        typeof newInterventionActive === "undefined" &&
        typeof environmentFlags === "undefined"
      ) {
        return;
      }
