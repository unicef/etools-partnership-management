 ...
 */
function FrNumbersConsistencyMixin<T extends Constructor<PolymerElement>>(
  superClass: T
) {
  class FrNumbersConsistencyClass extends EtoolsCurrency(
    ...
    frsConsistencyWarnings: GenericObject = {
      ...
      warningTmpl:
        "The <<frs_fields>> <<verb>> not the same as PD/SSFA <<pd_fields>>.",
    ...
    checkFrsConsistency(
      ...
      if (
        ...
        computedWarning = this.frsConsistencyWarnings
          .addedFrsCurrenciesMismatch;
      }
    ...
    validateFrsVsUnicefCashAmounts(
      ...
      if (
        ...
        (!skipEmptyListCheck &&
          this.emptyFrsList(intervention, interventionIsFromWhere))
      ) {
        return true;
      }
    ...
    emptyFrsList(
      intervention: Intervention | ListItemIntervention,
      interventionIsFromWhere: string
    ) {
