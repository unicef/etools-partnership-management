...
class ReportsList extends connect(store)(ListFiltersMixin(PolymerElement)) {
  ...
  stateChanged(state: RootState) {
    ...
    if (
      !isJsonStrMatch(this.reportStatuses, state.commonData!.reportStatuses)
    ) {
  ...
  _init(active: boolean) {
    ...
    this.setProperties({
      "queryParams.pd_ref_title": urlQueryParams.pd_ref_title
        ? urlQueryParams.pd_ref_title
        : "",
      "queryParams.external_partner_id": urlQueryParams.external_partner_id
        ? urlQueryParams.external_partner_id
        : null,
      "queryParams.cp_output": urlQueryParams.cp_output
        ? urlQueryParams.cp_output
        : null,
      "queryParams.section": urlQueryParams.section
        ? urlQueryParams.section
        : null,
      "queryParams.status": urlQueryParams.status
        ? urlQueryParams.status.split("|")
        : this.queryParams.status,
      "queryParams.unicef_focal_points": urlQueryParams.unicef_focal_points
        ? urlQueryParams.unicef_focal_points.split("|")
        : [],
      "queryParams.report_type": urlQueryParams.report_type
        ? urlQueryParams.report_type
        : null,
      "paginator.page": urlQueryParams.page ? Number(urlQueryParams.page) : 1,
      "paginator.page_size": urlQueryParams.page_size
        ? Number(urlQueryParams.page_size)
    ...
    if (!isEmptyObject(this.queryParams)) {
      Object.keys(this.queryParams).forEach((k: any) => {
        ...
        if (
          ...
          !isEmptyObject(this.queryParams[k])
        ) {
          qStrVal = this.queryParams[k].join("|");
        } else if (
          ["string", "number"].indexOf(typeof this.queryParams[k]) > -1
        ) {
