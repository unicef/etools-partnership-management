import {
  ListFilterOption,
  ListOrSelectedFilterOption,
} from "../../typings/filter.types";
...
function ListFiltersMixin<T extends Constructor<PolymerElement>>(baseClass: T) {
  class ListFiltersClass extends baseClass {
    ...
    _isAlreadySelected(filter: any) {
      const selectedOpt = this.selectedFilters.find(function (
        selectedFilter: ListFilterOption
      ) {
    ...
    selectFilter(e: DomRepeatEvent) {
      ...
      if (!this._isAlreadySelected(selectedOption)) {
        ...
        this.push(
          "selectedFilters",
          JSON.parse(JSON.stringify(selectedOption))
        );
        this.set(["listFilterOptions", idxInFilterOptions, "selected"], true);
      } else {
        // remove
        const idxInSelFilters = this.selectedFilters.findIndex(
          (f: any) => f.filterName === selectedOption.filterName
        );
        ...
        this._triggerUrlUpdateAndFiltering(
          this.listFilterOptions[idxInFilterOptions]
        );
    ...
    _triggerUrlUpdateAndFiltering(optionsFilter: any) {
      this.set(
        optionsFilter.path,
        this._getEmptyValueByFilterType(optionsFilter)
      );
    }
    ...
    _clearFilterSelection(
      ...
      this.clearSelectedValueInFilter(filter, [
        "listFilterOptions",
        idxInFilterOptions,
      ]);
      this.clearSelectedValueInFilter(filter, [
    ...
    clearAllFilterValues() {
      for (let i = 0; i < this.selectedFilters.length; i++) {
        if (!this.selectedFilters[i].disabled) {
          this.clearSelectedValueInFilter(this.selectedFilters[i], [
            "selectedFilters",
            i,
          ]);
        }
    ...
    clearSelectedValueInFilter(
      ...
      switch (filter.type) {
        ...
        case "etools-dropdown":
          this.set(
            [...filterPath, "selectedValue"],
            filter.singleSelection ? null : []
          );
          break;
        case "datepicker":
    ...
    esmmValueChanged(e: CustomEvent) {
      ...
      const filterVal = e.detail.selectedItems.map(
        (v: any) => v[dropdownMultiEl.optionValue!]
      );
      this.set(filterPath, filterVal);
    }
    ...
    _filterDateHasChanged(event: CustomEvent) {
      const filterPath = (event.target as DatePickerLite).getAttribute(
        "data-filter-path"
      )!;
    ...
    _validFilterSelectedValue(
      value: any,
      type: string,
      allowEmptyEsmm?: boolean
    ) {
    ...
    updateShownFilters(filtersToUpdate: ListOrSelectedFilterOption[]) {
      ...
      filtersToUpdate.forEach((filterToUpdate) => {
        ...
        const filterObj = this._findInListFilterOptions(
          filterToUpdate.filterName
        );
        ...
        if (
          this._validFilterSelectedValue(
            filterToUpdate.selectedValue,
            filterObj.type,
            filterObj.allowEmpty
          )
        ) {
          ...
          if (
            ...
            const idxInSelFilters = this.selectedFilters.findIndex(
              (f) => f.filterName === filterToUpdate.filterName
            );
            ...
            if (idxInSelFilters > -1) {
              ...
              this._disableFilterIfNeccessary(
                filterToUpdate,
                idxInSelFilters,
                idxInFilterOptions
              );
    ...
    _disableFilterIfNeccessary(
      ...
      if (filter.hasOwnProperty("disabled")) {
        this.set(
          ["selectedFilters", idxInSelFilters, "disabled"],
          filter.disabled
        );
      }
      ...
      this.set(
        ["listFilterOptions", idxInFilterOptions, "disabled"],
        filter.disableMenuOption
      );
    ...
    getFilterValuesByProperty(
      ...
      return filterOptions &&
        ...
        ? filterOptions
            .filter((opt) => selectedValues.indexOf(opt[selectedProp]) > -1)
            .map((opt) => opt[prop])
        : [];
    }
