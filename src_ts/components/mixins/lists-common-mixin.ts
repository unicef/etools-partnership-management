  ...
  class ListsCommonClass extends baseClass {
    ...
    connectedCallback() {
      ...
      this.addEventListener(
        "sort-changed",
        this._sortOrderChanged as EventListenerOrEventListenerObject
      );
    }
    ...
    _listDataChanged() {
      const rows = this.$.list.querySelectorAll(
        "etools-data-table-row"
      ) as NodeListOf<PolymerElement>;
      if (rows && rows.length) {
    ...
    _isValidSortField(fieldName: string) {
      // @ts-ignore
      return (
        this._sortableFieldNames instanceof Array &&
        this._sortableFieldNames.indexOf(fieldName) > -1
      );
    }
    ...
    _buildUrlQueryString(filters: GenericObject) {
      ...
      for (const field in filters) {
        if (filters[field]) {
          ...
          const filterValType =
            filterValue instanceof Array ? "array" : typeof filterValue;
          switch (filterValType) {
            ...
            case "object":
              if (
                ...
                filterUrlValue =
                  filterValue.field + "." + filterValue.direction;
              }
              break;
            default:
    ...
    _updateUrlAndDislayedData(
      currentPageUrlPath: string,
      lastUrlQueryStr: string,
      qs: string,
      filterData: () => void
    ) {
