...
const uploadStatus: Reducer<UploadStatusState, Action<string>> = (
  state = INITIAL_STATE,
  action: any
) => {
  switch (action.type) {
    ...
    case a.DECREASE_UNSAVED_UPLOADS: {
      const unsavedUploads =
        state.unsavedUploads > 0 ? state.unsavedUploads - 1 : 0;
      return {
        ...state,
        unsavedUploads,
      };
    ...
    case a.DECREASE_UPLOADS_IN_PROGRESS: {
      const uploadsInProgress =
        state.uploadsInProgress > 0 ? state.uploadsInProgress - 1 : 0;
      return {
        ...state,
        uploadsInProgress,
      };
