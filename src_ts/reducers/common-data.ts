...
const commonData: Reducer<CommonDataState, CommonDataAction> = (
  state = INITIAL_STATE,
  action: any
) => {
  let disaggregsCopy;
  ...
  switch (action.type) {
    ...
    case PATCH_DISAGGREGATION:
      ...
      dIndex = disaggregsCopy.findIndex(
        (disaggregsCopy) => disaggregsCopy.id === action.disaggregation.id
      );
      if (dIndex >= 0) {
...
const disaggregationsSelector = (state: RootState) =>
  state.commonData!.disaggregations;
